...
public class EntityFilter implements Comparable {
	...
	public Entity getBestEntityTarget(Entity entity) {
		...
		try {
			...
			for (int x = 0; x < entity.getMaxRange(); x++) {
				for (int y = 0; y < entity.getMaxRange(); y++) {
					for (int z = 0; z < entityMap[0][1].length; z++) {

						try {

							distancePoints = 100 / (calculateDifference(entityMap[x][y][z].getxPos(), x) + calculateDifference(entityMap[x][y][z].getyPos(), y) + 1);
	...
	public void sort(int xPosition, int yPosition) {
		...
		for (int row = 0; row < entityMap.length; row++) {
			for (int column = 0; column < entityMap[row].length; column++) {
				...
				try {
					...
					healthPoints = 100 / entityMap[row][column].getHealth();
		...
		Collections.sort(sortedUnitList, new Comparator<Entity>() {
			...
			public int compare(Entity entity1, Entity entity2) {
				...
				int value1 = entity1.getPriorityPoints();
				int value2 = entity2.getPriorityPoints();
