...
public class MapImageEffect extends BufferedImage {
	...
	// example command: "unitMove,attack=true,x1=10,x2=20,y1=10,y2=20"
	private void drawCommand(String command) {
		if(command.contains("unitMove")) {
			drawArrow(extractAttack(command),extractXOne(command), extractXTwo(command), extractYOne(command), extractYTwo(command));
		}
	}
